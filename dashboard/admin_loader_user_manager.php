    ...
    if (is_admin($c_uid))
    {
        if (isset($_POST["submit"]))
        {
            ...
            if (strlen($post_username) > 0 && strlen($post_password) > 0)
                update_user_by_uuid_and_gid($_POST["submit"], $_GET["gid"], $post_username, $post_password, $post_note, $post_active);
            echo '<script>window.location.href = "../backend/dashboard/redirect.php?filename=../../dashboard/admin_loader_user_manager.php?gid=' . $_GET["gid"] . '";</script>';
        }

        if (isset($_POST["submit_hwid"]))
        {
            set_attempt_hwid_as_new_hwid_by_gid_and_uuid($_GET["gid"], $_POST["submit_hwid"]);
            write_log("Admin: " . $dashboard_username. " updated HWID for user: " . $post_username);
            echo '<script>window.location.href = "../backend/dashboard/redirect.php?filename=../../dashboard/admin_loader_user_manager.php' . $_GET["gid"] . '";</script>';
        }
    }
